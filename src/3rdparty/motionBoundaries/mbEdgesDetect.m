function [E,O,inds,segs] = mbEdgesDetect( I, model )
...
%
% USAGE
%  [E,O,inds,segs] = mbEdgesDetect( I, model )
%
% INPUTS
...
if( opts.multiscale )
  ...
  for i=1:k, s=ss(i); I1=imResample(I,s);
    if(nargout<4), [E1,~,inds{i}]=mbEdgesDetect(I1,model);
    else [E1,~,inds{i},segs{i}]=mbEdgesDetect(I1,model); end
  ...
  [chnsReg,chnsSim] = mbEdgesChns( I, opts );
  s=opts.sharpen; if(s), I=convTri(rgbConvert(I,'rgb'),1); end
  if(nargout<4), [E,inds] = mbEdgesDetectMex(model,I,chnsReg,chnsSim);
  else [E,inds,segs] = mbEdgesDetectMex(model,I,chnsReg,chnsSim); end

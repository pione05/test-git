...
* Modified by Yang Hua and Henrique Morimitsu on Apr, 2016.
* This code is an adapted version that computes the scores only
* for a set of given boxes.
...
private:
  ...
  //add for input boxes
  void scoreAllInputBoxes(Boxes &boxes);
};
...
void EdgeBoxGenerator::generate( Boxes &boxes, arrayf &E, arrayf &O, arrayf &V )
{
  ...
  scoreAllInputBoxes(boxes);
}

void EdgeBoxGenerator::scoreAllInputBoxes( Boxes &boxes )
{
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  if(mxGetClassID(pr[2])!=mxSINGLE_CLASS) mexErrMsgTxt("Input boxes must be a float*");

  arrayf inputBoxes; inputBoxes._x = (float *) mxGetData(pr[2]);
  int noOfInputBoxes = (int) mxGetM(pr[2]);
  if (mxGetN(pr[2]) != 4) mexErrMsgTxt("The dimensions of input boxes must be Nx4");
  inputBoxes._h=noOfInputBoxes;
  inputBoxes._w=4;

  //printf("row number is %d, column number is %d\n", noOfInputBoxes, 4);

  Boxes boxes(noOfInputBoxes);
  for (int i = 0; i < noOfInputBoxes; ++i) {
    Box &b = boxes[i];
    b.c = round(inputBoxes.val(0, i)) - 1;   //column and row
    b.r = round(inputBoxes.val(1, i)) - 1;
    b.w = round(inputBoxes.val(2, i));
    b.h = round(inputBoxes.val(3, i));
  ...
  edgeBoxGen._alpha = float(mxGetScalar(pr[3]));
  edgeBoxGen._beta = float(mxGetScalar(pr[4]));
  edgeBoxGen._eta = float(mxGetScalar(pr[5]));
  edgeBoxGen._minScore = float(mxGetScalar(pr[6]));
  edgeBoxGen._maxBoxes = int(mxGetScalar(pr[7]));
  edgeBoxGen._edgeMinMag = float(mxGetScalar(pr[8]));
  edgeBoxGen._edgeMergeThr = float(mxGetScalar(pr[9]));
  edgeBoxGen._clusterMinMag = float(mxGetScalar(pr[10]));
  edgeBoxGen._maxAspectRatio = float(mxGetScalar(pr[11]));
  edgeBoxGen._minBoxArea = float(mxGetScalar(pr[12]));
  edgeBoxGen._gamma = float(mxGetScalar(pr[13]));
  edgeBoxGen._kappa = float(mxGetScalar(pr[14]));
